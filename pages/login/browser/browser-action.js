...
$(document).ready(function() {
	...
	$('#help').click(function() {
		chrome.tabs.update({
     		url: "http://sbpgarcia..io/sbp-login-autocomplete/"
		});
	});
	...
	$('#exportToFile').click(function() {
		slaUtils.getAllData(function(configuration) {
			chrome.runtime.sendMessage({
				action : 'copyToFile',
				content : JSON.stringify(configuration['sbp-sla'])
			});
		});
	});
	...
	$('#contact').click(function() {
		chrome.tabs.update({
     		url: "mailto:sbp.login.autocomplete@gmail.com?subject=[Contact]%20SLA-Extension&body=[Your message here]"
		});
	});
	...
	$('#fork').click(function() {
		chrome.tabs.update({
     		url: "https://.com/sbpgarcia/sbp-login-autocompleter"
		});
	});
...
function initBrowserActionDropdowns() {
	...
	slaUtils.getAllData(function(storedData) {
		_.each(storedData['sbp-sla'].environments, function(env) {
